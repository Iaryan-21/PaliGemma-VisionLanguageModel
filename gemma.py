...
class GemmaRotaryEncoding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        super().__init__()

        self.dim = dim # Each head will have it sown position encoding applied to as there will positional encoding to each head
        self.max_position_embeddings = max_position_embeddings # maximum number of position that we can encode
        self.base = base # base theta 

        inv_freq = 1.0/(self.base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).float()/ self.dim)) # 1/100000^(2i/d) # 0,2,4,6,8,: np.arange 
    ...
    def forward(self, x, position_ids, seq_len = None):
        ...
        with torch.autocast(device_type=device_type, enables=False): # Multi Automatted Mixed Precision 
            freqs = (inv_freq_expanded.float() @ position_ids_expanded.float()).transpose(1,2) # Multiplying each frequncy with each heading that we want to encode 
            emb = torch.cat((freqs, freqs),dim=1) # concatenate the cosine and sines [Batch_size, Seq_Len, Head_Dim]
            cos = emb.cos()
            sin = emb.sin()
...
def apply_rotary_pos_emb(q,k,cos,sin,unsqueeze_dim=1):
    ...
    q_embed = (q*cos)*(rotate_half(q)*sin)  # x1 * cosm(t1) + (-x1 {rotated})(sinm(t1))
    k_embed = (k*cos)*(rotate_half(k)*sin)  # x1 * cosm(t1) + (-x1 {rotated})(sinm(t1))

    return q_embed, k_embed 

def rotate_half(x):
    x1 = x[..., : x.shape[-1] // 2] # First Half of Head Dimension
    x2 = x[..., x.shape[-1] // 2 :] # Second Half of Head Dimension 
...
class GemmaAttention(nn.Module):
    def __init__(self, config:GemmaConfig, layer_idx:Optional[int] = None):
        ...
        self.rotary_emb = GemmaRotaryEncoding(
            self.head_dim,
